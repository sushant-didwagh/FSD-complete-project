...
const LoginAdmin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userData.role !== 'admin') {
        toast.error('This login is for admins only. Please use the correct login page.');
        return;
      }
      ...
      else if (status === 404) toast.error('No admin account found.');
  ...
  return (
    <div style={styles.page}>
      <div style={{ ...styles.orb, top: '15%', right: '20%', width: '350px', height: '350px', background: 'radial-gradient(circle, rgba(245,158,11,0.12) 0%, transparent 70%)' }} />
      <div style={{ ...styles.orb, bottom: '20%', left: '15%', width: '280px', height: '280px', background: 'radial-gradient(circle, rgba(245,158,11,0.08) 0%, transparent 70%)' }} />

      <div style={styles.container}>
        <div style={styles.headerSection}>
          <div style={{ ...styles.iconBox, background: `linear-gradient(135deg, ${accent}, ${accent}cc)`, boxShadow: `0 8px 32px rgba(245,158,11,0.35)` }}>
            🛡️
          </div>
          <h1 style={styles.title}>Admin Sign In</h1>
          <p style={styles.subtitle}>Manage platform, approve mentors & oversee operations</p>
        </div>
          ...
          <form onSubmit={handleSubmit} style={styles.form}>
            <div>
              ...
              <input
                ...
                placeholder="admin@studentbuddy.com"
                value={form.email}
            ...
            <button id="admin-login-submit" type="submit" className="submit-btn" disabled={loading}>
              {loading ? <span className="spinner" /> : '🛡️ Sign In as Admin'}
            </button>
          </form>

          {/* Restricted access note */}
          <div style={styles.noteBox}>
            🔒 <strong>Admin access</strong> is restricted to authorized personnel only.
          </div>
        </div>
      ...
      <style>{`
        ...
        .submit-btn {
          ...
          color: #1a0a2e; font-family: 'Inter', sans-serif; font-size: 1rem;
